...
ROOT_PATH = "D:/develop/workstations/GitHub/Datasets/DL/trained_outputs/RNN_music_composition_output/"
NOTES_PATH = ROOT_PATH + "notes_data/"
MIDI_PATH = ROOT_PATH + "music_midi/"
MUSIC_OUTPATH = ROOT_PATH + "music_output/"
...
def get_notes():
    ...
    print("----------create notes from %s *.mid!!!-------------" % MIDI_PATH)
    for midi_file in tqdm(glob.glob(MIDI_PATH + "*.mid")):
        midi_stream = converter.parse(midi_file)

        """
        获取所有乐器部分。
        # 注意：instrument.partitionByInstrument(midi_stream) 
    ...
    with open(NOTES_PATH + 'notes', 'wb') as filepath:
...
def create_music(prediction):
    ...
    # 写入 MIDI 文件
    midi_stream.write('midi', fp=MUSIC_OUTPATH + 'output.mid')

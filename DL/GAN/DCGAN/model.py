...
class DCGAN(object):
    def __init__(self, sess, input_height=108, input_width=108, is_crop=True,
                 ...
                 input_fname_pattern='*.jpg', work_dir=None, checkpoint_dir=None, sample_dir=None):
        """
    ...
    def train(self, config):
        ...
        if config.dataset == 'mnist':
            data_X, data_y = self.load_mnist(config.work_dir)
        else:
            data = glob(os.path.join(config.work_dir, "data", config.dataset, self.input_fname_pattern))
    ...
    def load_mnist(self, data_path):
        data_dir = os.path.join(data_path, "data", self.dataset_name)

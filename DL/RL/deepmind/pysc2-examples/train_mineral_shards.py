...
PATH_TO_MODEL = ROOT_PATH + "snapshot/mineral_shards/"
...
def main():
    ...
    if FLAGS.algorithm == "deepq-4way":
        logdir = FLAGS.log_path + "tensorboard/mineral/%s/%s_%s_prio%s_duel%s_lr%s/%s" % (
        ...
        logdir = FLAGS.log_path + "tensorboard/mineral/%s/%s_n%s_s%s_nsteps%s/lr%s/%s" % (
    ...
    if 'done' in locals and locals['done'] == True:
        if ('mean_100ep_reward' in locals and locals['num_episodes'] >= 10
            ...
            if not os.path.exists(os.path.join(PROJ_DIR, 'models/deepq-4way/')):
                try:
                    os.makedirs(os.path.join(PROJ_DIR, 'models/deepq-4way/'))
                except Exception as e:
                    print(str(e))
...
def a2c_callback(locals, globals):
    ...
    if ('mean_100ep_reward' in locals and locals['num_episodes'] >= 10
        ...
        if not os.path.exists(os.path.join(PROJ_DIR, 'models/a2c/')):
            try:
                os.makedirs(os.path.join(PROJ_DIR, 'models/a2c/'))
            except Exception as e:
                print(str(e))

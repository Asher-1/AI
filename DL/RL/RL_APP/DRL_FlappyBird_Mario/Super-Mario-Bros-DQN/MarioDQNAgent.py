...
class MarioDQNAgent(object):
    ...
    def build_network_copier(self):
        ...
        for idx, (predict_parameter, target_parameter) in enumerate(
                zip(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='prediction_network'),
            ...
            self.copied_parameters[predict_parameter.name] = tf.placeholder(tf.float32, shape=input_shape,
                                                                            name="copier_%d" % idx)
            self.copy_parameters_operation[predict_parameter.name] = target_parameter.assign(
                self.copied_parameters[predict_parameter.name])
    ...
    def run_minibatch(self):
        ...
        state, action, reward, state_prime, state_prime_is_terminal = self.replay_memory.get_batch_sample(
            self.batch_size)
        ...
        _, self.report_predictions, lr, self.one_hot_actions, self.final_predictions, self.report_loss = self.session.run(
            [self.optimizer, self.q_predictions, self.learning_rate, self.chosen_actions_one_hot, self.predict_y,
             self.loss], {
    ...
    def train(self):
            ...
            while not episode_terminated:
                ...
                if self.cur_state is not None:
                    self.replay_memory.add_to_memory(self.cur_state,
                                                     ...
                                                     # SARST has access no access to button maps
                                                     reward,
                                                     state_prime,
                                                     done)
        ...
        print("Mean episode rewards per %d timesteps are: \n %s" % (
            self.report_frequency, str(mean_episode_rewards[:self.episode_count])))

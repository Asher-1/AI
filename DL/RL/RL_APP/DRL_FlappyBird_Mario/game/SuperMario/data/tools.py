...
class Control(object):
    ...
    def __init__(self, caption, env=None):
    ...
    def get_step(self):
        ...
        reward, position_x = self.reward_policy()
    ...
    def reward_policy(self):
        ...
        reward = reward + score * self.additional_rewards['score']

        if self.state.mario.state == c.SMALL_TO_BIG:
            reward += self.additional_rewards['mushroom']
        elif self.state.mario.state == c.BIG_TO_FIRE:
            reward += self.additional_rewards['flower']
        elif self.state.mario.state == c.BIG_TO_SMALL:
            reward += self.additional_rewards['mushroom_hit']
        elif self.ml_done:
            reward += self.additional_rewards['death']
        elif self.state.mario.state == c.END_OF_LEVEL_FALL:
            reward += self.additional_rewards['finish_level']

        if self.keys[275] == 1:
            reward += self.additional_rewards['distance']
        else:
            reward += self.additional_rewards['idle']

        # if position_x < 70 and position_x != 0:
        #     self.ml_done = True
        return reward, position_x

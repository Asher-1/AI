...
# 迭代周期(原始为20，这里由于初始化网络参数时，使用了已经训练好的word2vec模型的网络参数进行初始化，收敛变快，训练周期可以适当小些，这里给：8)
...
# 仅用于随机初始化网络参数进行训练的模式: 即“不”使用已训练好模型初始化网络参数进行训练
# # Build vocabulary
# # 一行数据最多的词汇数
# max_document_length = max([len(x.split(" ")) for x in x_text])
    ...
    with sess.as_default():
        ...
        for batch in batches:
            ...
            if current_step % FLAGS.checkpoint_every == 0:
                checkpoint_prefix = r"/".join(checkpoint_prefix.split("\\"))
                path = saver.save(sess, checkpoint_prefix, global_step=current_step)
                print("Saved model checkpoint to {}\n".format(path))


# 训练模型可视化： tensorboard --logdir=runs\1521383715
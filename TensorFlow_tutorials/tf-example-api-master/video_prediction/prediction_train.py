...
OUT_DIR = 'G:/develop/PycharmProjects/datasets/result_outputs/video_prediction_output' + '/output_data/data'
...
flags.DEFINE_integer('checkpoint_step', 0,
                     'restore checkpoint iteration num.')
flags.DEFINE_integer('max_to_keep', 1,
                     'saved logs of max number.')
...
def main(unused_argv):
    ...
    saver = tf.train.Saver(
        tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES), max_to_keep=FLAGS.max_to_keep)
    ...
    if FLAGS.pretrained_model:
        ...
        # 只加载模型数据
        saver.restore(sess, latest_checkpoint_model)
        # model_name = os.path.basename(latest_checkpoint_model)
        # FLAGS.checkpoint_step = int(re.search(r"\d+", model_name).group())

        FLAGS.checkpoint_step = int(latest_checkpoint_model.split('/')[-1].split('-')[-1])
        print('restoring successfully from path: ', latest_checkpoint_model)
        print('from %d step start training...' % int(FLAGS.checkpoint_step + 1))

    tf.train.start_queue_runners(sess)

    tf.logging.info('iteration number, cost')
    # Run training.
    for itr in range(FLAGS.checkpoint_step + 1, FLAGS.num_iterations):
        ...
        print('iteration number: ', str(itr) + ' ', 'cost: ', str(cost))
        ...
        if itr % SAVE_INTERVAL == 2:
            ...
            print('.')
            saver.save(sess, FLAGS.output_dir + '/model.ckpt-' + str(itr))
            print('Saving model successfully to  ', FLAGS.output_dir + '/model.ckpt-' + str(itr))

        if itr % SUMMARY_INTERVAL == 2:
            summary_writer.add_summary(summary_str, itr)
            print('add summary successfully to path ', FLAGS.event_log_dir)

    tf.logging.info('Saving model.')
    print('Saving model to ', FLAGS.event_log_dir)
    saver.save(sess, FLAGS.output_dir + '/model.ckpt-' + int(FLAGS.num_iterations))

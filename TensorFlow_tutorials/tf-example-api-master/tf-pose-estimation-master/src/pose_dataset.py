...
class CocoPose(RNGDataFlow):
    ...
    def __init__(self, path, img_path=None, is_train=True, decode_img=True, only_idx=-1):
        ...
        if is_train:
            whole_path = os.path.join(path, 'person_keypoints_train2014.json')
        else:
            whole_path = os.path.join(path, 'person_keypoints_val2014.json')
        self.img_path = (img_path if img_path is not None else '') + ('train2014/' if is_train else 'val2014/')
...
def read_image_url(metas):
    img_str = None
    if 'http://' in metas.img_url:
        # print(meta.img_url)
        for _ in range(10):
            try:
                resp = requests.get(metas.img_url)
                if resp.status_code // 100 != 2:
                    logger.warning('request failed code=%d url=%s' % (resp.status_code, metas.img_url))
                    time.sleep(1.0)
                    continue
                img_str = resp.content
                break
            except Exception as e:
                logger.warning('request failed url=%s, err=%s' % (metas.img_url, str(e)))
    else:
        img_str = open(metas.img_url, 'rb').read()

    if not img_str:
        logger.warning('image not read, path=%s' % metas.img_url)
        raise Exception()

    nparr = np.fromstring(img_str, np.uint8)
    metas.img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    return metas

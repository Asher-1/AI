...
def work(data_dir, out_dir, landmarks, i):
    src_imname = os.path.join(data_dir, 'Img/img_celeba', '{:06d}.jpg'.format(i + 1))
...
def main(data_dir, out_dir, thread_num):
    ...
    with open(os.path.join(data_dir, 'Anno/list_landmarks_celeba.txt'), 'r') as f:
        landmarks = [list(map(int, x.split()[1:11])) for x in f.read().strip().split('\n')[2:]]

    im_list = glob.glob(os.path.join(data_dir, 'Img/img_celeba/*.jpg'))
...
if __name__ == '__main__':
    ...
    start = time.clock()
    main('../../../datasets/celebA/', '../../../datasets/celebA/align_5p/', 40)
    # 计时结束，并统计运行时长
    end = time.clock()
    print('预处理总时长：%f s --- %f min' % (end - start, (end - start) / 60.0))
